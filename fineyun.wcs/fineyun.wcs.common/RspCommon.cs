...
public class RspCommon
{
    ...
    /// 返回失败
    /// </summary>
    public static RspCommon Fail(int result = -1, string message = "")
    {
    ...
    public static RspCommonPaging<T> Pageing<T>(T data, long total) => new(data, total);
...
public class RspCommonPaging<T> : RspCommon
{
    ...
    public RspCommonPaging(T t, long total, int result = 1, string message = "")
    {
        Set(result, message);
        Total = total;
        Data = t;
